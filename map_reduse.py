...
def mapfn(k, v):
	...
	temp_list=v.split(":::")
	authors=temp_list[1].split("::")
	title=str(temp_list[2]).translate(None,badsym).strip("\n").lower().replace(" a ","")
...
for item in sorted(sorted(results.viewitems(),key=lambda x: x[1],reverse=True),key=lambda x: x[0][0]):
	print '{:.<15}{:.^15}{:.>15}'.format(item[0][0],item[0][1],item[1])
